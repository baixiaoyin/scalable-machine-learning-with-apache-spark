...
# MAGIC %md
# MAGIC XY: What is the syntax of the "translate" above? <br>
# MAGIC pyspark.sql.functions.translate(srcCol: ColumnOrName, matching: str, replace: str) → pyspark.sql.column.Column[source] <br>
# MAGIC A function translate any character in the srcCol by a character in matching. The characters in replace is corresponding to the characters in matching. The translate will happen when any character in the string matching with the character in the matching.
...
# MAGIC %md
# MAGIC Question: If I want to keep the average price group by minimum_nights, how can I do that?
...
display(imputed_df.sample(0.01))
